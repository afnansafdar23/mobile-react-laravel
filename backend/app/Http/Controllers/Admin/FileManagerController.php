...
class FileManagerController extends Controller
{
    ...
    {
        try {
            $fileManager = FileManager::create(['name' => $request->name]);
            foreach ($request->input('image', []) as $uploadedFile) {
                $fileManager->addMedia(storage_path('tmp/uploads/'.$uploadedFile))->toMediaCollection('file.image');
            }

            if ($fileManager) {
                $request->fileManager->move(public_path('storage/images'), $uploadedFile);
